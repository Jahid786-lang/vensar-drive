...
import { requestOtpThunk, verifyOtpThunk, loadPendingOtpThunk } from '@/store/authSlice'
...
export function LoginPage() {
  ...
  const from =
    (location.state && typeof location.state === "object" && "from" in location.state && location.state.from && typeof location.state.from === "object" && "pathname" in location.state.from)
  ...
  async function onCredentialsSubmit(data: LoginFormValues) {
    ...
    if (
      ...
      showToast('OTP sent successfully. Check your email or use the code below.', 'success')
    } else {
      const msg = (typeof result.payload === 'string' ? result.payload : 'Invalid credentials. Try again.')
  ...
  async function onOtpSubmit(data: OtpFormValues) {
    ...
    if (result.meta.requestStatus === 'fulfilled') {
      ...
      const msg = (result.payload as string) ?? 'Invalid OTP. Please try again.'
  ...
  return (
    <Box className="login-page" component="div">
      <Box className="login-illustration-wrap" component={motion.div} initial={{ opacity: 0.5 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
        <Box className="login-logo-pill" component={motion.div} initial={{ y: -8, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.15, duration: 0.5 }}>
              ...
              {!showOtpStep ? (
                ...
                >
                  <Box component="header" className="login-header">
                    <Typography component="h1" variant="h5" className="login-card-title">
                      Login
                    </Typography>
                    <Typography variant="body2" color="text.secondary" className="login-card-subtitle">
                      Sign in to continue
                    </Typography>
                  </Box>
                  ...
                  >
                    {credentialsForm.formState.errors.root && (
                      <Box component={motion.div} variants={itemVariants} role="alert">
                        <Alert
                          severity="error"
                    ...
                    <Box component={motion.div} variants={itemVariants} className="login-field">
                      <Controller
                        name="userId"
                    ...
                    <Box component={motion.div} variants={itemVariants} sx={{ mt: 0.5 }}>
                      <AppButton
                        type="submit"
                        variant="contained"
                        color="primary"
                    ...
                    <Box component={motion.div} variants={itemVariants} className="login-actions">
                      <Link
                        href="#"
                        variant="body2"
                        onClick={(e) => {
                ...
                >
                  <Box component="header" className="login-header">
                    ...
                    <Typography variant="body2" color="text.secondary" className="login-card-subtitle">
                      We sent a 6-digit code to <strong>{pendingLogin?.email}</strong>
                    </Typography>
                  </Box>
